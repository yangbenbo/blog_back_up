...
[Euler's laws of motion](https://browse.library.kiwix.org/viewer#wikipedia_en_all_maxi_2025-08/Euler's_laws_of_motion)是牛顿运动定理多粒子（特例是刚体）的扩展形式。**总体来说平动和转动可以解耦，外力作用到了质心产生平动（等效质量集中在质心的质点），绕通过质心转轴的转矩产生了转动（惯性空间任意轴，通过质心的轴，即使有加减速）**
...
- 第一定理：从某惯性参考系观测，施加于刚体的合外力，等于刚体质量与质心加速度的乘积
  ...
  >
  > **只依赖$$F=ma$$以及相互作用力，所以对任意物体的集合都适用，即使不是刚体**。比如刚开始保持静止的宇宙飞船中人往前走，则飞船轻微往后走，确保质心静止。再比如外太空火箭，没有空气情况下通过排出气体往前运动，也是类似的
  > $$
...
- 第二定理：设定某惯性参考系的固定点O（例如，原点）为参考点，施加于刚体的净外力矩，等于角动量的时间变化率
  ...
  > 1. 单质点转动定义转矩
  >
  > <img src="机械臂动力学/image-20260522070137359.png" style="zoom:80%;" />
  >
  > 物体转动，在转轴处建立坐标系
  > $$
  > \tau=r\times F=r\times \frac{dmv}{dt}=\frac{dr}{dt}\times mv+r\times \frac{dmv}{dt}=\frac{d(r\times mv)}{dt}  =\frac{d(-m[r]^2w)}{dt}=\frac{d(Iw)}{dt}=\frac{dL}{dt}=I\alpha
  > $$
  > 这里也可以使用能量来定义转矩，平动可看成转动中心无限远的转动
  > $$
  > \frac{dE}{dt} =  \frac{d\frac{1}{2}mv^2}{dt}=F^Tv=F^T[w]r=-F^T[r]w=([r]F)^Tw=\tau^Tw=\tau^T\frac{d\theta}{dt}\\
  > $$
  > 2. 刚体转动相对惯性系：所有单个粒子进行分析，内力抵消后剩余每个粒子的外力矩，和为总外力矩
  >
  > $$
  > \sum_{i=1}^n\frac{dL_i}{dt}=\sum_{i=1}^n(r_i\times F_i)=\sum_{i=1}^n(r_i\times F_{ext})=\sum_{i=1}^n\tau_{i\_ext}=\tau_{ext}
  > $$
  >
  > 3. 刚体转动相对质心系
  >
  ...
  >
  > 绳子提重物的质心不会有转动，惯性力本身可等效成重力。所以转到质心得到，同样**适用任意物体集合，包括刚体**
  > $$
...
$$
F_{ext}=\frac{dp_{cm}}{dt}=\frac{d}{dt}m(v+[w]r_{cm})=\frac{d}{dt}(mv+m[r_{cm}]^Tw)
$$

$$
\tau_{ext} =\sum_i\frac{dL_{i}}{dt} = \sum_i\frac{d}{dt}m_i(r_i\times (v+[w]r_{i})) = \frac{d}{dt}(m[r_{cm}]v+Iw) \\
$$

> 这里的$w,v$都是旋量，代表刚体的速度，即刚体固连且与基坐标系重合点的角速度和线速度
>
> 微分内容分别对应线动量和角动量
...
$$
\left\{\begin{matrix}
 \vec{\dot{q}_1^0}=\dot{\theta_1}\vec{s}_1^0+\color{red}{H_1^{-1}\mathrm{Ad}(T_{sb}){\vec{\dot{q}}_{parent}}}&i=1\\
\vec{\dot{q}_i^0}=\dot{\theta_1}\vec{s}_1^0+H_i^{-1}\vec{\dot{q}_i^0}&i>1
\end{matrix}\right.
$$

$$
\left\{\begin{matrix}
 \vec{\ddot{q}_1^0}-\vec{g_1^0}=\ddot{\theta_1}\vec{s}_1^0+\dot{\theta_1}\left [ \vec{\dot{q}_1^0},\vec{s_1^0} \right ]+\color{red}{H_1^{-1}\left(\mathrm{Ad}(T_{sb}){(\vec{\ddot{q}}_{parent}}-\vec{g})\right)}&i=1\\
\vec{\ddot{q}_i^0}-\vec{g_i^0}=\ddot{\theta_1}\vec{s}_1^0+\dot{\theta_i}\left [ \vec{\dot{q}_i^0},\vec{s_i^0} \right ]+H_i^{-1}(\vec{\ddot{q}_i^0}-\vec{g}_{i-1}^0)&i>1
\end{matrix}\right.
$$
...
$$
\left\{\begin{matrix}
 \vec{P_n^0}=\vec{Q_n^0} +\color{red}{\mathrm{Ad}(T_{bs}^T)^{-1}(H_1^{T})^{-1}{\vec{P}_{child}}}&i=n \\
 \vec{P_i^0}=\vec{Q_i^0}+(H^T_{i+1})^{-1}P_{i+1}^0 &i<n
\end{matrix}\right.
$$
...
$$
\left\{\begin{matrix}
 \vec{P_n^0}=\vec{Q_n^0} +\color{red}{\mathrm{Ad}(T_{0b}^T)^{-1}{\color{blue}\sum_{m}} (H_1^{T})^{-1}{\vec{P}_{child}}}&i=n \\
 \vec{P_i^0}=\vec{Q_i^0}+(H^T_{i+1})^{-1}P_{i+1}^0 &i<n
\end{matrix}\right.
$$
...
刚体转动动能计算
$$
T = { \frac { 1 } { 2 } } \! \sum m _ { i } v _ { i } ^ { 2 } = { \frac { 1 } { 2 } } \! \sum m _ { i } \left( \, r _ { i } \omega \right) ^ { 2 } = { \frac { 1 } { 2 } } \omega ^ { 2 } \sum m _ { i } r _ { i } ^ { 2 } = { \frac { 1 } { 2 } } I \omega ^ { 2 }
$$
和动能非常类似 

 

滑冰运动员在冰面自转，身体展开转速小，身体收缩（如手收回来）转速增加，看起来可以用能量守恒分析，但实际上是角动量守恒，忽略冰面的摩擦力，人只受到冰面的支持力和自身重力，没有相对于轴的转动力矩，所以角动量守恒，$$I_1w_1=I_2w_2$$，角动量变小，则角速度变大，发现动能增加了，似乎能量不守恒。事实并不是这样，能量增加是因为人做功了，手收回过程中克服离心力做功，对于身体未收缩部分分析发现角动量增加，即受到了外力矩，这里就是**科里奥利力**
$$
\tau = F _ { c } r = \frac { d L } { d t } = \frac { d \left( m \omega r ^ { 2 } \right) } { d t } = 2 m \omega r \frac { d r } { d t } = 2 m \omega v \cdot r
$$
对应有科式力就是$$2mwv$$

在转动圆台原点附近容易理解科式力：沿着径向跨过原点，实际绝对位置是一条圆弧，所以肯定受到一个力改变速度的方向 

![](机械臂动力学/image-20251218092419359.png)

在匀速运动圆台上人沿着切向匀速运动$$v_M$$，则有$$v_J=v_M+𝜔r$$，所以实际人受到的力分成3个部分，其中包括 
$$
F _ { r } = - \, \frac { m v _ { \mathrm { J } } ^ { 2 } } { r } = \frac { m v _ { \mathrm { M } } ^ { 2 } } { r } - 2 m v _ { \mathrm { M } } \omega - m \omega ^ { 2 } \, r
$$
从这个角度来说可以把力分解成沿着径向的力和垂直径向的力，分别进行计算（沿着轴向的力不产生科式力）
