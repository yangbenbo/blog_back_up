...
- 终值定理
$\mathcal{L}(f')=\int_0^{+\infty}f's^{-st}dt=sF(s)-f(0)$
取极限得到$\lim_{s \to 0} \int_0^{+\infty}f's^{-st}dt=f(+\infty )-f(0)=\lim_{s \to 0}sF(s)-f(0)$, 所以$f(+\infty )=\lim_{s \to 0}sF(s)$
...
## 伯德图
伯德图就是不同频率下chuachua传递函数幅值和相位的响应
...
通过观察发现高频部分输入的幅值被有效抑制,对应低通滤波器.$G=\frac{1}{s+1}$就是一阶巴特沃斯滤波器(Butterworth Filter)
...
分析传递函数幅值求最大值对应频率为$w=w_n\sqrt{1-2\xi ^2}$, 即为共振频率, 对应需要满足$\xi < \sqrt{0.5}$, 当$\xi$很小时就是约等于固有频率. 当$\xi=0$时, $|G|=\frac{w_n^2}{w_n^2-w^2}$, 当$w=w_n$时, $|G|\to +\infty $, 系统不稳定, 符合临界稳定(根在虚轴上)但不满足BIBO稳定
...
- 积分器, $G=\frac{1}{s}$
如下图所示,发现在频率很低处输出幅值比输入大, 需要额外能量, 所以积分器无法通过被动元件实现, 需要额外供能
